...
## UI Polish
- [ ] Bar screen: weave in purple accent from brand palette (e.g. `colors.brand.plum`) for selected pills, section headers, or subtle card accents to give the bar its own identity.
- [ ] Garden screen: weave in a greener accent for garden-specific elements (suggestion cards, status badges, section styling) to differentiate from the rest of the app.
