...
Deno.serve(async (req) => {
  ...
  try {
    ...
    const { meal_name, description, existing_names, ingredients: plannedIngredients } = input;
...
1. List the ingredients needed (quantities + units where helpful, e.g. "300g chicken thighs", "2 cloves garlic"). If planned ingredients are provided, use them as your ingredient list — they are the exact items the user has planned to buy. Add any missing basics but do not substitute or reinvent.
...
Respond ONLY with valid JSON matching this exact schema:
{
  ...
  "glossary": [{"term": "string", "definition": "string"}]
      ...
      messages: [
        {
          role: 'user',
          content: `Meal name: ${meal_name}\nDescription: ${description ?? ''}${plannedIngredients ? `\n\nPlanned ingredients (use these exact items and quantities — do not reinvent the dish):\n${plannedIngredients}` : ''}`,
        },
      ],
    });
