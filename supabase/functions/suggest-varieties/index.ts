...
Deno.serve(async (req) => {
  ...
  try {
    ...
    const response = await client.messages.create({
      ...
      max_tokens: 256,
      system: `You suggest garden plant varieties for a home gardener in Canterbury, New Zealand (Southern Hemisphere, ~43.5°S). Given a plant name, return 4-6 varieties that grow well in Canterbury's climate. Favour varieties available from NZ seed suppliers (Kings Seeds, Egmont Seeds, Koanga). Respond ONLY with valid JSON: {"varieties": ["Variety 1", "Variety 2", ...]}`,
